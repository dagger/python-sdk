...
- every local dependency **registered to this SDK** — read from the workspace
  ...
  because `asSDK.modules` is cwd-scoped (`core/schema/module_as_sdk.go:78`)
...
modules on this repository's `runtime/` by relative path; `app` declares `dep`
twice, once aliased `greeter`, and its source imports its own client from the
start):
...
- `standaloneClientCheck` — `generateClient` for `dep` produces a
  ...
  normalized engine version; `initClient` seeds only `pyproject.toml`; the
  registered-client rollup materializes a client and is empty on a second run.
- `standaloneRuntimeCheck` — a script using the generated client runs under
  `uv run` in a Python-capable runner, from a workdir inside the workspace,
  with `_EXPERIMENTAL_DAGGER_CLI_BIN` pointing at the harness's CLI so the
  check is deterministic and needs no egress: it opens its own session,
  serves `dep` by workspace path and prints its answer. The stamp itself is
...
- `foreignDependencyCheck` — a fixture depending on this repository's Go
  ...
  engine's staging and fails loudly in the value workspace, rather than being
  silently skipped; it pins which branch a foreign dependency takes.
...
What stays untested, plainly: a module consumed from git (needs a published
