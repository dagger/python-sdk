...
from codegen.partition import PartitionError, client_module_name
...
wrap = partial(textwrap.wrap, break_long_words=False)
...
CORE_ALIAS = "_core"
"""How a client module refers to the core API package."""

BINDING_NAME = "_BINDING"
"""The module-level name of a client's ModuleBinding."""

ENTRY_CLIENT_PARAM = "client"
"""The entry point's keyword-only parameter selecting the client to use."""

_body_reserved = frozenset(["_ctx", "_args", CORE_ALIAS, BINDING_NAME])
"""Names a client function's body uses, which a schema argument may not shadow."""
...
"""Module-level names a client file already binds, which nothing rendered may take."""


@dataclass(frozen=True)
class Binding:
    """The identity a client is bound to; rendered verbatim into the client."""
...
class Context:
    ...
    mode: str = CORE_MODE
    """What is being rendered: the core API, or one module's client."""

    module: str = ""
    """In client mode, the bound module's final name."""

    binding: Binding | None = None
    """In client mode, the identity the client serves its module under."""

    owned_types: frozenset[TypeName] = field(default_factory=frozenset)
    """In client mode, the types the bound module contributes."""

    owned_fields: frozenset[tuple[TypeName, FieldName]] = field(
        default_factory=frozenset
    )
    """In client mode, the fields the bound module contributes to core types."""
    ...
    def is_client(self) -> bool:
        return self.mode == CLIENT_MODE

    def renders_type(self, name: TypeName) -> bool:
        """Whether this file defines the named type."""
        return not self.is_client or name in self.owned_types

    def type_ref(self, name: TypeName) -> str:
        """How this file refers to a named type.

        Bare when this file defines it, through the core alias otherwise.
        """
        if not self.is_client:
            return name
        if name in self.owned_types:
            return name
        if (
            name.startswith("_")
            and name.endswith("Client")
            and name[1:-6] in self.owned_types
        ):
            return name
        return f"{CORE_ALIAS}.{name}"
...
def generate(  # noqa: PLR0913
    ...
    *,
    ...
    module: str = "",
    binding: Binding | None = None,
    owned_types: frozenset[TypeName] = frozenset(),
    owned_fields: frozenset[tuple[TypeName, FieldName]] = frozenset(),
) -> Iterator[str]:
    """Code generation main function.

    In core mode the whole schema is rendered as the core API, ending in the
    ``Client`` root and the ``dag`` singleton. In client mode only what the
    bound module owns is rendered — its types, its entry point on ``Query``
    and its functions on other core types — and every other type is referred
    to through the core alias.
    """
    if mode == CLIENT_MODE and (not module or binding is None):
        msg = "client mode needs the bound module's name and binding"
        raise ValueError(msg)

    yield textwrap.dedent(CLIENT_HEADER if mode == CLIENT_MODE else CORE_HEADER)
    ...
    if ctx.is_client:
        ...
        yield ""
        yield f"{BINDING_NAME} = ModuleBinding("
        yield indent(f"name={ctx.binding.name!r},")
        yield indent(f"kind={ctx.binding.kind!r},")
        yield indent(f"ref={ctx.binding.ref!r},")
        yield indent(f"pin={ctx.binding.pin!r},")
        yield ")"
        yield ""
    ...
    if ctx.legacy_sdk_compat and not ctx.is_client:
    ...
    types_n, types_g = itertools.tee(
        get_grouped_types(handlers, schema.type_map, ctx.renders_type)
    )
    ...
    if ctx.is_client:
        for shim in client_functions(ctx):
            yield shim.render()
            ctx.defined.add(shim.name)
    else:
        yield ""
        yield ""
...
def get_grouped_types(
    handlers: tuple[Handler, ...],
    type_map: TypeMap,
    include: Callable[[TypeName], bool] = lambda _: True,
):
    """Group types by handler and sorted by their name."""

    def _filtered():
        for n, t in type_map.items():
            if n.startswith("_") or is_builtin_scalar_type(t) or not include(n):
                continue
...
TypeRef: TypeAlias = Callable[[TypeName], str]


def _bare(name: TypeName) -> str:
    return name
...
def format_input_type(
    ...
    ``ref`` renders a named type the way the file being generated refers to
    it; builtin scalars and ``Type`` are always bare.
    """
    if is_required_type(t):
        t = t.of_type
        fmt = "%s"
    ...
    if is_list_type(t):
        inner = format_input_type(t.of_type, convert_id, expected_type, legacy_ids, ref)
        return fmt % f"list[{inner}]"

    if is_id_type(t):
        if convert_id:
            if expected_type is not None:
                return fmt % ref(expected_type)
    ...
    if is_scalar_type(t):
        return fmt % (
            Scalars.from_type(t) if is_builtin_scalar_type(t) else ref(t.name)
        )
    return fmt % ref(get_named_type(t).name)
...
def format_output_type(
    ...
    if not is_output_leaf_type(t) and not is_required_type(t):
        t = GraphQLNonNull(t)
    return format_input_type(t, False, expected_type, legacy_ids, ref)
...
def doc(s: str) -> str:
    ...
    s = s.replace("\\", "\\\\")
    # Escaped before the block quotes, while no quote in the string is escaped
    # yet, so a trailing one is unambiguously a delimiter in the making.
    if s.endswith('"'):
        s = f'{s[:-1]}\\"'
    s = s.replace('"""', '\\"\\"\\"').replace("\r", "\\r").replace("\x00", "\\x00")
    if "\n" in s:
        s = f"{s}\n"
    return f'"""{s}"""'
...
class _InputField:
    ...
    def __init__(
        ...
        self.is_self = (
            self.parent_object_name is not None
            and self.type == ctx.type_ref(self.parent_object_name)
        )
        ...
        if default_value and is_enum_type(self.named_type):
            enum_ref = ctx.type_ref(self.named_type.name)
            self.default_value = f"{enum_ref}.{default_value}"
        else:
...
class _ObjectField:
    ...
    def __init__(
        ...
        if (
            ...
            self.type = ctx.type_ref(self.expected_type)
    ...
    def func_signature(self) -> str:
        ...
        ret_type = (
            "Self" if self.type == self.ctx.type_ref(self.parent_name) else self.type
        )
    ...
    def func_body(self) -> Iterator[str]:
        ...
        if deprecated := self.deprecated():
            msg = f'Method "{self.name}" is deprecated: {deprecated}'
            for char, escaped in (
                ("\\", "\\\\"),
                ('"', '\\"'),
                ("\n", "\\n"),
        ...
        yield self.select_line()

        if not self.is_exec:
            # Use the concrete client class for interface types
            yield f"return {self.object_class()}(_ctx)"
        elif self.is_list:
            yield f"return await _ctx.execute_object_list({self.object_class()})"
        elif self.is_void:
            yield "await _ctx.execute()"
        else:
            yield f"return await _ctx.execute({self.type})"

    def select_line(self) -> str:
        return f'_ctx = self._select("{self.graphql_name}", _args)'

    def object_class(self) -> str:
        """The class to build for an object return.

        The concrete client class for an interface, the type itself otherwise.
        """
        name = self.named_type.name
        if is_interface_type(self.named_type):
            name = f"_{name}Client"
        return self.ctx.type_ref(name)
...
class _ClientFunction(_ObjectField):
    """A module-owned field on a core type, rendered as a module-level function.

    ``Query.<module>`` is the client's entry point: it takes the module's
    constructor arguments plus a keyword-only ``client`` and starts a new
    chain with the binding attached. Any other core parent (``Binding``,
    ``Env``, …) becomes a function taking that parent as its first argument.
    """
    ...
    def __init__(
        ...
        self.is_entry = self.parent_name == "Query"
        self.parent_param = format_name(self.parent_name)
        if self.convert_id:
            msg = (
                f"function {self.parent_name}.{self.graphql_name!r} of module "
                f"{ctx.module!r} returns its parent's ID: a module-level function "
                "has no object to chain the result from"
            )
            raise PartitionError(msg)
        reserved = _body_reserved | {
            ENTRY_CLIENT_PARAM if self.is_entry else self.parent_param
        }
        for arg in self.args:
            if arg.name in reserved:
                arg.name = f"{arg.name}_"
        names = [arg.name for arg in self.args]
        if len(set(names)) != len(names):
            dupes = sorted({n for n in names if names.count(n) > 1})
            msg = (
                f"function {self.parent_name}.{self.graphql_name!r} of module "
                f"{ctx.module!r} has arguments that normalize to the same Python "
                f"name: {', '.join(dupes)}"
            )
            raise PartitionError(msg)
    ...
    def render(self) -> Iterator[str]:
        yield ""
        yield self.func_signature()
        yield indent(self.func_body())
        yield ""

    def func_signature(self) -> str:
        params: list[str] = []
        if not self.is_entry:
            params.append(f"{self.parent_param}: {self.ctx.type_ref(self.parent_name)}")
        params.extend(a.as_param() for a in self.required_args)
        params.append("*")
        params.extend(a.as_param() for a in self.default_args)
        if self.is_entry:
            client = (
                f"{ENTRY_CLIENT_PARAM}: {self.ctx.type_ref('Client')} | None = None"
            )
        ...
        joined = ", ".join(params)
        # arbitrary heuristic to force trailing comma in long signatures
        if len(joined) > 40:  # noqa: PLR2004
            joined = f"{joined},"
        sig = self.ctx.render_types(f"def {self.name}({joined}) -> {self.type}:")
        if self.is_exec:
            sig = f"async {sig}"
        return sig
    ...
    def select_line(self) -> str:
        if self.is_entry:
            root = (
                f"{CORE_ALIAS}.dag if {ENTRY_CLIENT_PARAM} is None "
                f"else {ENTRY_CLIENT_PARAM}"
            )
            chain = f"({root})._ctx.with_binding({BINDING_NAME})"
            select = f'root_select("{self.graphql_name}", _args)'
        else:
            chain = f"{self.parent_param}._ctx.with_binding({BINDING_NAME})"
            select = f'select("{self.parent_name}", "{self.graphql_name}", _args)'
        return f"_ctx = {chain}.{select}  # noqa: SLF001"

    def func_doc(self) -> str:
        if not self.is_entry:
            return super().func_doc()
        intro = "\n".join(
            wrap(
                "Executing any query through the returned object serves the "
                "module into the session first; a module bound by workspace path "
                "needs a workspace, a module bound by git ref resolves anywhere. "
                f"Pass `{ENTRY_CLIENT_PARAM}` to use a connection other than the "
                "global one."
            )
        )
        head = f"Client for the `{self.ctx.module}` module.\n\n{intro}"
        doc = super().func_doc()
        return f"{head}\n\n{doc}" if doc else head


def client_functions(ctx: Context) -> list[_ClientFunction]:
    """The module-level functions of a client, entry point first.

    A function on a core type is always named after its parent and field, so
    that adding a field to another parent never renames an exported one. The
    entry point is named after the module, like the file it lives in.
    """
    functions: list[_ClientFunction] = []
    entry_name = client_module_name(ctx.module)
    for parent, field_name in sorted(ctx.owned_fields):
        parent_type = cast(GraphQLObjectType, ctx.schema.get_type(parent))
        graphql_field = parent_type.fields[field_name]
        name = (
            entry_name
            if parent == "Query"
            else f"{format_name(parent)}_{format_name(field_name)}"
        )
        functions.append(
            _ClientFunction(ctx, field_name, graphql_field, parent_type, name)
        )

    entries = [f for f in functions if f.is_entry]
    if len(entries) != 1:
        msg = (
            f"module {ctx.module!r} contributes {len(entries)} Query fields, "
            "expected exactly 1"
        )
        raise PartitionError(msg)

    _reject_name_collisions(ctx, functions)
    functions.sort(key=lambda f: (not f.is_entry, f.name))
    return functions


def _reject_name_collisions(ctx: Context, functions: list[_ClientFunction]):
    """Every name a client file binds is its own: nothing shadows anything."""
    for name in sorted(ctx.owned_types | {f.name for f in functions}):
        if name in _module_reserved:
            msg = (
                f"module {ctx.module!r}: {name!r} is a name its client file "
                "already binds"
            )
            raise PartitionError(msg)

    taken: set[str] = set(ctx.owned_types)
    for function in functions:
        if function.name in taken:
            msg = (
                f"module {ctx.module!r}: function {function.name!r} collides "
                "with another name in its client"
            )
            raise PartitionError(msg)
