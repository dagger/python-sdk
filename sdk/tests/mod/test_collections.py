...
def test_collection_metadata_reaches_both_entrypoints():
    from dagger.mod._entrypoint import render_types
    from dagger.mod._module import _object_from
    ...
    class Main:
        ...
        @mod.function(name="lookup")
        @mod.get
        def item(self, key: str) -> Item:
            return Item(name=key)

    desc = mod.describe()
    obj = next(obj for obj in desc.objects if obj.name == "Main")
    assert obj.collection
    assert next(f for f in obj.fields if f.name == "paths").collection_role == "keys"
    assert next(f for f in obj.functions if f.name == "lookup").collection_get
    code = render_types(desc)
    assert ".withCollection" in code
    assert '.withCollectionKeys("paths")' in code
    assert '.withCollectionDelta("selection")' in code
    assert '.withCollectionGet("lookup")' in code
    # Build the runtime registration query without an engine connection.
    runtime = _object_from(obj)
